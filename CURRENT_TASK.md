...
- 3A-R1.2 最新结论：local passed / Actions failed。已确认安装包 payload 有效，根因是默认 per-user 安装目录在当前中文用户名环境下没有稳定落盘；已通过 `build/installer.nsh` 固定默认安装目录为 `%LOCALAPPDATA%\Programs\AIWorkbench`，并新增 `scripts/verify-nsis-install.mjs`。本地 `npm.cmd run verify:install-release` 已通过，证据见 `verification/install-release/repair1-2-summary.json` 和 `verification/install-release/preflight-summary.json`。GitHub Actions Run `29919498085` 失败在 `Build installer candidate`，preflight 未执行；已补 workflow 诊断让下一次 run 上传 build log。
