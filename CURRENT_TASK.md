...
- 只验证现有生存体检半成品，补齐缺失证据，写回权威文档并交付。
- 不从头重跑成本调查或计算，不调用真实模型，不产生模型费用。
- 不修改功能代码、Managed Proxy、Cloudflare、模型配置、Release、安装包或用户数据。
- 不实现平台月度总开销上限、自动熔断、模型分层、上下文压缩、v0.4.7 或其他后续任务。
- 完成后停止，等待产品负责人验收生存体检。
...
当前唯一下一步以 `NEXT_STEP.md` 为准：等待产品负责人验收生存体检。未经批准，不得实现平台月度总开销上限、自动熔断或其他后续任务。

完成本轮后必须停止，等待产品负责人验收生存体检，不自动进入平台月度总开销上限、自动熔断、第二批清理或其他任务。
