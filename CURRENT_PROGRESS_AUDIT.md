...
- 现在卡在什么：上线三大硬骨头已完成。3A-R1.3、3A-R2.0、3A-R2.1、③A 总验收和 ③B GitHub Alpha Release 均已 passed；公开 Release 下载回测确认安装包大小和 SHA256 与 ③A 候选包完全一致。产品方向已收口并写入现有文档。DeepSeek V4 Flash 非思考兼容 version 13 已通过真实 Preview 链路：注册 1 次、聊天 1 次、HTTP 200、provider model `deepseek-v4-flash`、预算 23 micro-USD 写入平台总账和 V4 Flash 明细。唯一下一步是等待产品负责人验收；未经批准不得把 version 13 加入 production deployment、切换 1%/100% 或发起新的主动真实模型调用。
- `research/` 里真实存在文件：见第 2 节，共 12 个 `.md` 文件。
- `research/` 里应该有但缺的文件：`market-intelligence.md`，原因见第 3 节。

## 5. 近期优先级

1. 等待产品负责人验收 version 13 DeepSeek V4 Flash 非思考真实 Preview 链路。
2. 后续 production deployment、1% 灰度或 100% 全量切换只能在产品负责人明确批准后执行。
3. 模型分层调度与上下文压缩。
...
- 成本失控：生存体检已确认当前钱包安全状态 unsafe；第 3A 本地钱包刹车已通过，生产 D1 预算表已创建，DeepSeek V4 Flash 非思考真实 Preview 链路已通过，但 version 13 尚未加入 production deployment，灰度或全量仍未完成；模型分层和上下文压缩仍未完成。
