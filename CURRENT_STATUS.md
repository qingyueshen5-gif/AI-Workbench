...
- **HIGH · LEGACY_WORKBENCH_RUN_VERIFIED_TRUST_BOUNDARY_BYPASS**：旧Workbench Run API、整库JSON回灌、孤立Run验证和相关上下文传播链允许未经权威Task、Run、验收revision、真实Verifier PASS及成功Final完整绑定的外部值进入、保存或传播为业务`verified`。状态=`OPEN`；severity=`HIGH`；deploymentImpact=`BLOCKED`；followUp=`LEGACY-RUN-API-VERIFIED-TRUST-BOUNDARY-FIX-001`。批准基线六处证据为`server.mjs:206`（`normalizeRuns()`/`PUT /api/data`整库回灌）、`:366`（`buildTaskContextPackage()`传播）、`:1358`（旧Workbench工具执行）、`:1863`（`/api/hermes/invoke`）、`:2047`（`POST /api/runs`外部写入）和`:2116`（孤立Run验证）。完整链包括外部写入口、JSON持久化、Agent上下文、UI整库回写、旧测试依赖、孤立Run验证、Gateway/Delivery待核验边界及历史存量裸`verified=true`。该风险独立于前三项内部误用风险；外部调用方可直接提交或回灌可信状态，因此严重度更高。迟到审计及WIP证据见`verification/VERIFIED-SEMANTICS-UNIFICATION-001/fourth-risk-late-audit.md`与Checkpoint `VERIFIED-SEMANTICS-UNIFICATION-FOURTH-RISK-WIP-001`。
