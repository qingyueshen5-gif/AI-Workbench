...
当前尚无真实用户。模型调用成本由平台承担，第 3 阶段钱包刹车已正式封板，平台模型调用 40 USD 月度硬上限已在生产生效；自然用户长期规模稳定性仍未证明。v0.4.7 当前只完成可执行施工图审计，尚未进入功能开发。当前目标是建立可靠产品、真实用户、知名度和可持续经营能力，不是收费或利润最大化。
...
不得上传新 Worker version、注册 installation、发起真实模型调用、修改 Secrets、进入后续段、实际电脑清理、首屏示例、反馈入口、安全告知、真实用户测试、模型分层、上下文压缩、手机端、情报流水线或任何新功能开发，除非产品负责人明确批准对应实施工作包。
...
v0.4.7 市场基础可用性阶段共 5 步：代码审计和施工图、公共底层、隔离并行开发、Codex 统一集成并内部验收、1–2 名陌生真人下载安装和真实环境验收。当前只完成第 1 步施工图，不代表功能开发已启动。

真实代码审计结论以 `CURRENT_PROGRESS_AUDIT.md` 的“v0.4.7 可执行施工图”为权威。当前代码已有 Windows 桌面壳、本机 `8787` 应用服务、`18800` 模型代理、Managed Proxy、DeepSeek 当前生产链路、基础会话列表、本地数据文件、环境 readiness、错误归一化、Hermes/OpenClaw adapter 雏形、安装包和部分验证脚本。尚未实现市场可用的图片/文件输入、反馈入口、产品埋点、隐私告知、预算撞顶专门展示、统一附件消息结构、会话恢复验收、真实安装真人验收和内部手机指挥入口。

推荐第一批只等待产品负责人审核，不得自动开工：工作包 A 公共底层/统一输入和模型抽象，工作包 E 反馈/埋点/错误日志和隐私，工作包 G 测试/虚拟人格和质量验收。工作包 B 基础界面、C 图片文件、D 上下文会话、F 安装环境恢复、H 内部研发提速通道均需在对应底层或产品拍板后进入。
