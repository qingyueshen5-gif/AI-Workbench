...
| Hermes 仍打印日志/状态数据库权限错误 | 2026-07-17 已定位：旧版 AI Link watchdog 会自动拉起另一套 `%USERPROFILE%\.hermes\hermes-agent\.venv` 里的 Hermes gateway，和当前 Windows/Anaconda 版 Hermes 抢 `.__agent.lock`、`auth.lock` 和 `state.db`；另外 Codex 沙盒内非授权运行 Hermes 时，本身不能写 `%USERPROFILE%\AppData\Local\hermes`，也会看到权限错误 | 已处理：只关闭 AI Link watchdog 中的 Hermes gateway 自动拉起逻辑，保留 bingling 代理和 AI Link 模型服务自愈；停止旧 gateway，清理 lock，并给 `CodexSandboxUsers` 补 Hermes 状态目录修改权限。2026-07-17 用真实 Windows 用户权限运行 `hermes doctor` 已无 `.__agent.lock` 和 `session DB readonly` 报错。注意：在 Codex 沙盒里不带授权直接跑 Hermes，仍可能因沙盒写入限制报 AppData 权限错误；验证 Hermes 环境应使用系统终端或授权运行 |
