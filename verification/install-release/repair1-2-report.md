...
- 失败步骤：Fail when preflight failed
- 说明：build 成功，preflight 已执行，但最终仍 failure；当前 `gh` token invalid，无法读取日志（HTTP 403）或下载 artifact（HTTP 401），因此本环境无法取得云端 preflight 失败详情。
