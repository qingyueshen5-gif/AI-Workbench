...
长期愿景不是只服务中国或海外某一个市场，而是让不同国家和地区的普通用户都能用同一个简单入口获得可靠结果。地区差异可以由后台适配，用户不应该为了语言、模型、代理、平台规则和工具选择反复配置。
...
产品不能只在用户会提问、状态好、逻辑清楚时才有效。睡眠、情绪、记忆和判断质量会波动，工作台必须保存长期目标、产品初衷、历史决策和当前进度；当用户的新决定与原方向冲突时，先提醒冲突和代价，再等待确认，而不是盲目执行。
...
跨平台执行的长期标准是在用户授权和平台规则范围内，像可靠的人类助理一样操作网站和电脑：阅读、收集、比较、填写、下载、上传、中断恢复和跨平台交接。它不是绕过验证码、安全限制、平台权限或反自动化规则的工具。
...
长期愿景可以同时存在，但每项都必须标明当前最小可执行步骤、前提、现实风险、验证方法和状态，避免把愿景写成已完成事实。

| 长期方向 | 当前最小可执行步骤 | 所需前提 | 现实风险 | 验证方法 | 当前状态 |
| --- | --- | --- | --- | --- | --- |
| 全球普通用户 | 先用 v0.4.6/v0.4.7 证明普通用户能安装、启动、输入目标并得到结果。 | 首批真人试用、基础隐私告知、稳定安装链路。 | 语言、支付、网络、合规和支持成本差异。 | 不同地区用户试用反馈和错误日志。 | 长期愿景，未启动全球化开发。 |
| 专业用户 | 找到高时间杠杆用户的重复任务，验证是否真正省时间。 | 首批专业用户样本和可复核交付证据。 | 需求复杂、质量要求高、失败成本更高。 | 任务完成时间、返工次数、满意度。 | 候选验证方向。 |
| 团队和开发者 | 先保持个人工作台稳定，再研究团队权限和 API。 | 单人版稳定、权限模型、数据隔离。 | 多人协作会拉高复杂 UI 和权限风险。 | 团队试点和权限审计。 | 长期能力，当前不做。 |
| 模型/Agent/工具生态 | 用统一 adapter 接入真正能解决用户问题的能力。 | 调度层、质量检查、成本监管和失败恢复稳定。 | 接入过多导致质量失控和维护成本膨胀。 | 每个接入能力的验收和真实用户使用证据。 | 长期方向，按真实需求排序。 |
| 多语言、多地区和多平台 | 先记录真实用户地区和语言需求，不提前铺开。 | 真实用户数据、隐私合规、分发策略。 | 翻译质量、平台审核、地区法规。 | 用户来源、失败率、支持请求。 | 等待真实用户反馈。 |
| 国际多渠道入口 | 先把飞书/微信/Telegram/WhatsApp/Discord 统一定义为可替换入口适配器。 | 统一任务入口、身份权限、成本核算和产品负责人选择首个试验入口。 | 单一平台锁死、地区规则差异、账号和消息隐私风险。 | 最小闭环验证：手机发任务、工作台接收、权限检查、执行、回原入口。 | 战略已锁定，首选入口待批准，当前不开发。 |
| 降低优秀 AI 能力进入生活和工作的门槛 | 保持一个输入框，把复杂模型和工具调度留在后台。 | 基础体验、成本安全、权限透明。 | 为展示能力而把复杂度推给用户。 | 首次任务完成率和用户是否理解下一步。 | 当前核心方向，持续约束。 |
| 加密支付 | 只研究真实支付问题、法律允许性、平台规则和成熟替代方案。 | 明确国家/用户支付痛点、合规意见和资产安全方案。 | 法律、欺诈、资产安全、退款争议和用户教育风险。 | 合规审查和真实支付需求证据。 | 战略研究，现在不做。 |
