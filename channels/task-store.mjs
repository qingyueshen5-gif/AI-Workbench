...
const runAllowed=new Map([
  ['created',new Set(['starting','failed','cancelled','superseded'])],
  ['starting',new Set(['running','failed','cancelled','superseded'])],
  ['running',new Set(['verifying','failed','cancelled','superseded'])],
  ['verifying',new Set(['completed','failed','cancelled','superseded'])],
  ['completed',new Set()],['failed',new Set()],['cancelled',new Set()],['superseded',new Set()]
]);
...
export class TaskStore {
  ...
  async transitionRun(taskId,{runId,taskRevision,from,to,evidence=null}={}){return exclusive(`${this.root}:${taskId}`,async()=>{const task=await this.load(taskId);if(!task)throw new Error(`Task not found: ${taskId}`);if(task.activeRunId!==runId)throw new Error(`Stale Run: ${runId}`);if(task.taskRevision!==taskRevision)throw new Error(`Task revision conflict: expected ${taskRevision}, actual ${task.taskRevision}`);const index=task.runs.findIndex((run)=>run.runId===runId);if(index<0)throw new Error(`Run not found: ${runId}`);const run=task.runs[index];if(run.taskRevision!==taskRevision)throw new Error(`Run revision conflict: expected ${taskRevision}, actual ${run.taskRevision}`);if(run.status!==from)throw new Error(`Run state conflict: expected ${from}, actual ${run.status}`);if(!RUN_STATES.includes(to)||!runAllowed.get(from)?.has(to))throw new Error(`Illegal Run transition: ${from} -> ${to}`);const at=this.now();const updated={...run,status:to,startedAt:to==='starting'&&!run.startedAt?at:run.startedAt,heartbeatAt:at,finishedAt:TERMINAL_RUN_STATES.has(to)?at:null,...(evidence?{providerEvidence:evidence}:{})};const runs=[...task.runs];runs[index]=updated;const next={...task,runs,updatedAt:at,evidence:[...(task.evidence||[]),{type:'run_transition',runId,from,to,taskRevision,at,evidence}]};return this.save(next);});}
