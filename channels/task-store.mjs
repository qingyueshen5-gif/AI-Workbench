...
export class TaskStore {
  ...
  async patch(taskId,patch){const task=await this.load(taskId);if(!task)throw new Error(`Task not found: ${taskId}`);for(const forbidden of ['currentState','stateReason','terminalAt','stateHistory'])if(Object.hasOwn(patch,forbidden))throw new Error(`Task state field requires transitionTask: ${forbidden}`);if(Object.hasOwn(patch,'originalMessageId')&&patch.originalMessageId!==task.originalMessageId)throw new Error('originalMessageId is immutable');return this.save({...task,...patch,taskId,originalMessageId:task.originalMessageId,updatedAt:this.now()});}
  async transitionTask(taskId,from,to,reason,actor,evidence){const task=await this.load(taskId);if(!task)throw new Error(`Task not found: ${taskId}`);if(task.currentState!==from)throw new Error(`Task state conflict: expected ${from}, actual ${task.currentState}`);if(!TASK_STATES.includes(to)||!allowed.get(from)?.has(to))throw new Error(`Illegal task transition: ${from} -> ${to}`);required(reason,'reason');required(actor,'actor');if(evidence===undefined||evidence===null)throw new Error('transition evidence is required');const at=this.now();const next={...task,currentState:to,stateReason:reason,updatedAt:at,terminalAt:TERMINAL_TASK_STATES.has(to)?at:null,stateHistory:[...(task.stateHistory||[]),{from,to,reason,actor,evidence,timestamp:at}],evidence:[...(task.evidence||[]),{type:'state_transition',from,to,reason,actor,evidence,at}]};return this.save(next);}
