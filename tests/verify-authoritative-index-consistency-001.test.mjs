...
test('T1 current real index is Step7 PRESENT_AND_VERIFIED at 7/7',async()=>{const r=run(),o=jsonOut(r),index=JSON.parse(await readFile(realIndex,'utf8'));assert.equal(r.status,0,JSON.stringify(o.failures));assert.equal(o.ok,true);assert.equal(index.steps.step6.machineStatus,'PRESENT_AND_VERIFIED');assert.equal(index.steps.step7.machineStatus,'PRESENT_AND_VERIFIED');assert.equal(index.remainingWork.completionRatio,'7/7');assert.equal(index.remainingWork.step8Eligible,true);assert.ok(index.remainingWork.presentAndVerified.includes('STEP7'));assert.ok(!index.remainingWork.actuallyMissing.includes('STEP7'));});
...
test('T10 added evidence/finalAcceptance fields do not produce PASS',()=>expectFailure(async({index})=>{index.steps.step5E.evidence={invented:true};index.finalAcceptance=false;},'INDEX_NO_STEP5E_EVIDENCE'));
