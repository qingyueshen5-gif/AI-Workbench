...
test('T1 current real index is Step6 PRESENT_AND_VERIFIED at 6/7',async()=>{const r=run(),o=jsonOut(r),index=JSON.parse(await readFile(realIndex,'utf8'));assert.equal(r.status,0,JSON.stringify(o.failures));assert.equal(o.ok,true);assert.equal(index.steps.step6.machineStatus,'PRESENT_AND_VERIFIED');assert.equal(index.remainingWork.completionRatio,'6/7');assert.ok(index.remainingWork.presentAndVerified.includes('STEP6'));assert.ok(!index.remainingWork.actuallyMissing.includes('STEP6'));});
