export class ResultVerifier {
  ...
  verifyCapabilityResult(capabilityId, result) {
    ...
    if (capabilityId === 'runtime.status') {
      const sources = Array.isArray(result.evidence?.evidenceSources) ? result.evidence.evidenceSources : [];
      if (!String(result.text || '').trim() || !result.evidence?.readAt || !sources.length || sources.some((item) => item?.read !== true || !item?.readAt || !item?.sourceId || !item?.sourceType)) throw new Error('runtime.status缺少真实可验证的状态来源');
    }
