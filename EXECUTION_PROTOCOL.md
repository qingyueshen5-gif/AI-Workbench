...
### DISPLAY-LINE-NUMBER-PREFIX-MUST-NOT-BE-WRITTEN

1. `view`、日志或代码展示中的前导行号仅用于显示，不属于文件内容。
2. 从展示输出生成补丁前，必须剥离前导空格、显示行号以及紧随其后的制表符或显示分隔符。
3. 禁止将“行号 + 源代码”整体写回文件；有效代码自身的数字、字符串数字和缩进不得被误删。
4. 修改任何 `.mjs` 后必须立即执行 `node --check <file>`。
5. 语法检查失败时必须先恢复结构完整性，不得继续叠加功能修改。
6. 发现显示前缀进入文件时，分类为 `TOOL_OUTPUT_PREFIX_CONTAMINATION`。
7. 修复前必须先保存受污染文件、diff、语法结果和SHA-256证据。
