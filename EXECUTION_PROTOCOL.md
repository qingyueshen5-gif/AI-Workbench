...
v0.4.7 当前处于施工图审核阶段，不等于功能开发已经开始。

任何 v0.4.7 工作包开工前必须写清：
...
任一项为空，状态必须写 `not_ready_to_start`，不得开工。

推荐第一批候选工作包只能在产品负责人批准后启动。工作包可以并行研究或隔离开发，但所有涉及 `src/main.jsx`、`server.mjs`、`model-proxy.mjs`、`runtime-paths.mjs`、`errors/normalize.mjs`、`electron/main.cjs`、安装脚本、验证脚本和当前状态文档的修改，最终必须由 Codex 统一集成、统一测试、统一形成候选版本。

v0.4.7 工作包不得默认修改 `managed-proxy/src/`、Cloudflare 配置、D1 schema、Secrets、生产流量、预算额度、模型路由、package version、Release 或安装包。确需触碰这些边界时，必须新任务单独批准。

## 3.2 本地 Codex 任务网关规则

本地 Codex 任务网关只用于内部研发任务调度，不是对外产品功能。

所有真实 Codex 任务必须满足：

- 先创建结构化任务卡；
- 状态先进入 `waiting_approval`；
- 产品负责人或当前执行者显式 approve 后才允许 run；
- 每个代码任务使用独立 branch 和独立 worktree；
- 不直接修改主工作区；
- 不直接修改 main；
- 不自动 push；
- 不自动部署；
- 不读取或复制 Codex、AI Link、飞书或其他登录凭据；
- 不在命令行参数中暴露 Secret；
- 任务日志必须脱敏。
...
- 同时运行 Codex 任务数默认 1；
- 可配置但不得超过 2；
- 同一仓库同一 `concurrency_group` 同时只能运行 1 个；
- 两个任务不得共用 worktree 或 branch。

渠道边界：

- LocalCLIChannel 和未来 FeishuChannel 只负责接收命令、展示状态、请求人工批准和展示摘要；
