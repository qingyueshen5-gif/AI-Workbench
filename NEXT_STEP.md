...
- 第 3B-2b2a 段 Preview 版本已上传并验证，当前等待产品负责人验收。

## 为什么停在 3B-2b2a 验收

- 本轮只上传已锁定候选为新的 Cloudflare Worker Preview version，并验证 Preview URL。
- 本轮确认 active deployment 和生产流量完全不变，远端预算表仍为空。
- 本轮未部署 Cloudflare Worker、未修改 Secrets、未使用真实安装 Token、未调用真实 provider。
- Preview 版本已上传，但生产钱包刹车尚未生效；Worker 部署属于后续段，必须等待产品负责人验收 3B-2b2a 并明确批准。

## 本轮允许范围

当前只允许：

- 等待产品负责人验收第 3B-2b2a 段 Preview 上传验证；
- 查阅 `verification/monthly-budget-worker-preview-upload/summary.json` 和 `report.md`；
- 如产品负责人提出验收问题，只回答本轮证据范围内的问题。

禁止：

- 部署 Cloudflare Worker；
- 执行 `wrangler deploy` 或 `wrangler versions deploy`；
- 修改 Worker 流量；
- 执行 rollback；
...
- 写入预算表；
- 修改生产 D1；
- 自动进入第 3B-2b2b 段、模型分层、上下文压缩或 v0.4.7；
...
验收批准后的候选下一步是“第 3B-2b2b 段：Worker 部署和生产验证”。它不是本轮已执行内容，未经产品负责人批准不得开始。
