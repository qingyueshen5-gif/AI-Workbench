...
import { ContractSessionStore, ContractTaskStore, ContractToolExecutor, ContractModelRouter, ContractVerifier, ContractProgressWriter, verifyAgentRuntimeDependencyContracts } from './runtime-dependency-contract-fixtures.mjs';
import { taskInterpretations } from './task-interpreter-contract-fixtures.mjs';
...
const deps={
 sessions:new ContractSessionStore(),tasks:new ContractTaskStore(),
 ...
 models:new ContractModelRouter({understand:async()=>({text:JSON.stringify(taskInterpretations.chat('ok'))}),express:async()=>({text:'ok'}),execute:async()=>({text:'executed',sessionId:'s'})}),
