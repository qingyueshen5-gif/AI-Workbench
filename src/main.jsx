    ...
    <main className="bg-white text-zinc-950">
      <div className="workbench-shell w-full">
        ...
        <MobileConversationDrawer
          ...
          setOpen={setConversationPanelOpen}
          data={data}
...
function MobileConversationDrawer({ open, setOpen, data, updateData }) {
  return (
    <>
      {open && <button className="mobile-conversation-backdrop" aria-label="关闭对话列表" onClick={() => setOpen(false)} />}
      <ConversationList
        data={data}
        updateData={updateData}
        onSelect={() => setOpen(false)}
        className={`mobile-conversation-panel border-r border-zinc-200 bg-zinc-50 ${open ? 'mobile-conversation-panel-open' : ''}`}
      />
    </>
  );
}
  ...
  return (
    <aside className={className}>
      <div className="flex h-16 items-center justify-between px-4">
        ...
        <div className="rounded-md border border-zinc-300 px-2 py-1 text-xs text-zinc-500">v{appVersion}</div>
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-3 pb-4">
        ...
        <div className="mb-2 px-2 text-xs font-semibold text-zinc-500">最近对话</div>
        ...
        {!!days.length && (
          <div className="mt-6">
            <div className="mb-2 px-2 text-xs font-semibold text-zinc-500">历史记录</div>
            <div className="space-y-1">
              {days.map((day) => (
    ...
    <header className="flex h-16 shrink-0 items-center justify-between border-b border-zinc-100 px-5">
      <div className="flex min-w-0 items-center gap-3">
        <button
          ...
          className="conversation-switch-button flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-zinc-200 hover:bg-zinc-100 md:hidden"
          aria-label="切换对话"
        >
          <ConversationIcon />
        </button>
      ...
      <button
        ...
        className="ml-4 flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-zinc-200 hover:bg-zinc-100"
        aria-label="打开侧边面板"
      >
        <MenuIcon />
      </button>
        ...
        <div className="mx-auto flex min-h-full max-w-3xl flex-col justify-end py-8">
          ...
          {messages.map((message) => (
            <article key={message.id} className={message.role === 'assistant' ? 'group' : 'group text-right'}>
              <div className={message.role === 'assistant' ? 'message-timestamp mb-1 text-xs text-zinc-400' : 'message-timestamp mb-1 text-right text-xs text-zinc-400'}>{timeText(message.createdAt)}</div>
